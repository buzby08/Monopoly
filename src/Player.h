...
enum class PlayerType { User, Ai, AIWait };
/**
...
public:
    ...
    [[nodiscard]] int GetOutOfJailAttempts() const;
    [[nodiscard]] bool IsBankrupt() const { return bankrupt; }
    [[nodiscard]] PlayerType GetPlayerType() const { return player_type; }

    virtual GetOutOfJailOption get_out_of_jail_option() = 0;
    virtual bool buy_property(Property property) = 0;
    virtual std::string request_option(const std::string &message) = 0;

};

template<class Archive>
void Player::serialize(Archive &ar) {
    ar(player_type, id, color, space, money, get_out_of_jail_attempts, get_out_of_jail_free, in_jail, bankrupt);
}
