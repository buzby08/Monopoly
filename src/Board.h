...
class Board {
    ...
    int get_property_index(int space_number) const;

    std::unordered_map<int, std::string> read_spaces(const std::string& file_path);
    
    void initialise(const std::string &new_board_file_path, const std::string &new_properties_file_path);
    
    std::vector<std::unique_ptr<Property>> get_properties(const std::string &file_path);
...
public:
    ...
    std::vector<Property*> get_properties_by_player(int player_id) const;
    ...
    Property* get_space_property(int space_number) const;
    std::string get_space_string(int space_number);
    ...
    void mark_owned(int space_number, Player* new_owner) const;
    bool is_owned(int space_number) const;
    std::vector<Property*> get_properties_by_owner(int owner_id) const;
};

template <class Archive>
void Board::serialize(Archive &ar) {
    ar(board_file_path, properties_file_path, properties);
}
